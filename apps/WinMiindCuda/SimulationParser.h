...
public:
	...
	void addGridAlgorithmGroupNode(pugi::xml_document& doc, std::string alg_name) {
	...
	void addRateFunctorNode(pugi::xml_document& doc, std::string alg_name) {
		for (pugi::xml_node algorithm = doc.child("Simulation").child("Algorithms").child("Algorithm"); algorithm; algorithm = algorithm.next_sibling("Algorithm")) {
	...
	void parseXmlFile() {
		...
		for (pugi::xml_node node = doc.child("Simulation").child("Nodes").child("Node"); node; node = node.next_sibling("Node")) {
			...
			*/
			...
			addGridAlgorithmGroupNode(check_doc, algorithm_name);
			addRateFunctorNode(check_doc,algorithm_name);

			_node_ids[node_name] = _node_ids.size();
		}
