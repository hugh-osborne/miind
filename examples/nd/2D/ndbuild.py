...
class MiindNDFileGenerator:
    def __init__(self, _grid, _basename, _timescale, _reset_v, _threshold_v):
        self.grid = _grid
    ...
    def coords_to_index(self, coords):
        index = 0
        multiplicand = np.prod(self.grid.resolution)
        for d in range(len(self.grid.resolution)):
            multiplicand /= self.grid.resolution[d]
            index += int(coords[d] * multiplicand)
        return index
        
    def coords_to_strip_and_cell(self, coords):
        index = 0
        multiplicand = np.prod(self.grid.resolution)
        for d in range(len(self.grid.resolution)-1):
            multiplicand /= self.grid.resolution[d]
            index = index + (coords[d] * multiplicand)
    ...
    def generateTMatFile(self, mc = False, count = 10):
        ...
        with open(self.basename + '.tmat', 'w') as tmat_file:
            ...
            for c in transitions:
                (to_row_num, to_cell) = self.coords_to_strip_and_cell(c[0])

                line = '1000000000;' + str(to_row_num) + ',' + str(to_cell) + ';'
                for t in c[1]:
                    (from_row_num, from_cell) = self.coords_to_strip_and_cell(t[0])

                    line += str(from_row_num) + ',' + str(from_cell) + ':' + '{:.9f}'.format(t[1]) + ';'
                line += '\n'
...
# def resetFunc(w):
#     return min(-35,-65 + (w * 50.0))

# def resetFuncW(w):
#     return -((1.0 - w) * 0.01)

# g = Grid([-150.0,-0.2], [150.0,1.2], [300,200], rybak, -29.0, 0.1)
# two_d_gen = MiindGrid2DFileGenerator(g, 'rybak', 0.001, -57.0, -29.0, -0.01, resetFunc, resetFuncW)
# two_d_gen.generateModelFile()
# two_d_gen.generateTMatFile()

# g = Grid([-200.0,-0.2], [200.0,1.2], [300,200], rybakPF, -15.0, 0.1)
# two_d_gen = MiindGrid2DFileGenerator(g, 'rybakPF', 0.001, -57.0, -15.0, -0.01, resetFunc)
# two_d_gen.generateModelFile()
# two_d_gen.generateTMatFile()
...
# g = Grid([-100.0,-0.2,-0.2], [150.0,1.2,1.2], [20,20,20], rybak_3d, -29.0, 0.1)
# n_d_gen = MiindNDFileGenerator(g, 'rybak_3d', 0.001, -57.0, -15.0)
# n_d_gen.generateModelFile()
# n_d_gen.generateTMatFile()
# n_d_gen.generateTMatFile(True,100)
