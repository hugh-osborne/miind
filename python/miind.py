...
def generate_preamble(fn, vars, nodes, algorithms, connections, parameters, cuda):
    ...
    with open(fn,'w') as f:
        ...
        f.write('#include <TwoDLib/MeshNd.hpp>\n\n')
        if cuda == True: f.write('typedef CudaTwoDLib::fptype fptype;\n')
        f.write('\n')
        variables.parse_variables(vars, f)
        f.write('\n')
     ...
     with open(fn,'a') as f:
         ...
         for node in nodes:
              ...
              if algorithm.attrib['type'] == 'MeshAlgorithmGroup':
                  ...
                  f.write('\tpugi::xml_node  root' + str(node_id) + ' = doc.first_child();\n\n')
                  #f.write('\tTwoDLib::Mesh mesh' + str(node_id) +' = TwoDLib::RetrieveMeshFromXML(root' + str(node_id) + ');\n')
                  f.write('std::vector<double> dim_' + str(node_id) + '(2);\n')
                  f.write('dim_' + str(node_id) + '[0] = 60.0;\n')
                  f.write('dim_' + str(node_id) + '[1] = 40;\n')
                  f.write('std::vector<unsigned int> res_' + str(node_id) + '(2);\n')
                  f.write('res_' + str(node_id) + '[0] = 50*50;\n') # 50*50 is two dimensions (plus res[0] makes 3)
                  f.write('res_' + str(node_id) + '[1] = 200;\n') 
                  f.write('std::vector<double> base_' + str(node_id) + '(2);\n')
                  f.write('base_' + str(node_id) + '[0] = -2.0;\n')
                  f.write('base_' + str(node_id) + '[1] = -75;\n')
                  f.write('\tTwoDLib::MeshNd mesh' + str(node_id) +' = TwoDLib::MeshNd(0.00001,2,res_' + str(node_id) + ',dim_' + str(node_id) + ',base_' + str(node_id) + ');\n')
                  f.write('\tstd::vector<TwoDLib::Redistribution> vec_rev' + str(node_id) + ' = TwoDLib::RetrieveMappingFromXML("Reversal",root' + str(node_id) + ');\n')
                  f.write('\tstd::vector<TwoDLib::Redistribution> vec_res' + str(node_id) + ' = TwoDLib::RetrieveMappingFromXML("Reset",root' + str(node_id) + ');\n\n')
