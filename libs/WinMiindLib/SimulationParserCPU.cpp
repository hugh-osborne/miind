...
void SimulationParserCPU<MPILib::CustomConnectionParameters>::parseXmlFile() {
	...
	if (std::string(TypeParseTraits<MPILib::CustomConnectionParameters>::name) != std::string(doc.child("Simulation").child_value("WeightType"))) {
		std::cout << "The weight type of the SimulationParser (" << TypeParseTraits<MPILib::CustomConnectionParameters>::name << ") doesn't match the WeightType in the XML file (" << doc.child("Simulation").child_value("WeightType") << "). Exiting.\n";
		return;
	}
	...
	_algorithms = map<std::string, std::unique_ptr<MPILib::AlgorithmInterface<MPILib::CustomConnectionParameters>>>();
	...
	for (pugi::xml_node conn = doc.child("Simulation").child("Connections").child("Connection"); conn; conn = conn.next_sibling("Connection")) {
		...
		// too lazy right now...
		if (TypeParseTraits<MPILib::CustomConnectionParameters>::name == std::string("CustomConnectionParameters"))
	...
	SimulationRunParameter par_run(*MiindTvbModelAbstract<MPILib::CustomConnectionParameters, MPILib::utilities::CircularDistribution>::report_handler, (simulation_length / time_step) + 1, 0,
